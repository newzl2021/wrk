...
lua_State *lua_script_create(char *file, char *url, char **headers) {
...
void lua_script_init(lua_State *L, thread *t, int argc, char **argv) {
    lua_getglobal(t->ctx.L, "wrk");

    script_push_thread(t->ctx.L, t);
    lua_setfield(t->ctx.L, -2, "thread");
    ...
    lua_getfield(t->ctx.L, -1, "init");
    lua_newtable(t->ctx.L);
    for (int i = 0; i < argc; i++) {
        lua_pushstring(t->ctx.L, argv[i]);
        lua_rawseti(t->ctx.L, -2, i);
    }
    lua_call(t->ctx.L, 1, 0);
    lua_pop(t->ctx.L, 1);
}
...
void lua_script_response(lua_State *L, int status, buffer *headers, buffer *body) {
...
void lua_script_summary(lua_State *L, uint64_t duration, uint64_t requests, uint64_t bytes) {
    const table_field fields[] = {
...
void lua_script_errors(lua_State *L, errors *errors) {
    uint64_t e[] = {
        errors->connect,
        errors->read,
        errors->write,
...
void lua_script_done(lua_State *L, stats *latency, stats *requests) {
...
size_t lua_script_verify_request(lua_State *L) {
    ...
    lua_script_request(L, &request, &len);
...
static int script_thread_get(lua_State *L) {
    ...
    lua_getglobal(t->ctx.L, key);
    script_copy_value(t->ctx.L, L, -1);
    lua_pop(t->ctx.L, 1);
    return 1;
}
...
static int script_thread_set(lua_State *L) {
    ...
    script_copy_value(L, t->ctx.L, -1);
    lua_setglobal(t->ctx.L, name);
    return 0;
}
